	...
	return function multiSelectWorksModule (QUnit) {
		QUnit.module( 'multi', {}, function testDownKeyPresses () {
			...
			QUnit.test('adds and removes aria-selected="true" to/from selected items', function loadTree (assert) {
				...
				assert.notOk($toSelect1.attr('aria-selected'), 'item 1 does not contain aria-selected attr');
				assert.notOk($toSelect2.attr('aria-selected'), 'item 2 does not contain aria-selected attr');
				...
				assert.notOk($toSelect2.attr('aria-selected'), 'item 2 still does not contain aria-selected attr');
