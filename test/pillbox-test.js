...
define(function(require){
	...
	test("should behave as designed", function () {
		...
		deepEqual($pillbox.pillbox('items')[0], {name: 'Item 1', value: 'foo'}, 'pillbox returns item data');
	});
	...
	test("Input functionality should behave as designed", function () {
		...
		deepEqual($pillbox.pillbox('items')[5], {name: 'three-value', value: 'three-value'}, 'pillbox returns added item');
	});
	...
	test("addItems/removeItems functions", function(){
		...
		$pillbox.pillbox('addItems', {name:'Item 2', value:2});
		...
		deepEqual($pillbox.pillbox('items')[1], {name: 'Item 2', value: 2}, 'multiple items have been added correctly');

		$pillbox.pillbox('removeItems');

		$pillbox.pillbox('addItems', [{name:'Item 1', value:1},{name:'Item 2', value:2},{name:'Item 3', value:3}]);
		deepEqual($pillbox.pillbox('items')[2], {name: 'Item 3', value: 3}, 'multiple items have been added correctly by array');

		$pillbox.pillbox('removeItems',2,1);
		deepEqual($pillbox.pillbox('items')[1], {name: 'Item 3', value: 3}, 'single item has been removed at the correct index');
		$pillbox.pillbox('removeItems',1);
		deepEqual($pillbox.pillbox('items')[0], {name: 'Item 3', value: 3}, 'single item has been removed at the correct index');
	});
	...
	test("removeByValue function", function(){
		...
		deepEqual($pillbox.pillbox('items')[0], {name: 'Item 2'}, 'item not removed has correct text and value');
	});
	...
	test("removeByText function", function(){
		...
		deepEqual($pillbox.pillbox('items')[0], {name: 'Item 1', value: 'foo'}, 'item not removed has correct text and value');
	});
	...
	test("all user defined methods work as expected", function(){
		...
		deepEqual($pillbox.pillbox('items')[2], {name: 'Item 3', value: 'three-value'}, 'item correctly added after onAdd user callback');
	...
	test("Suggestions functionality should behave as designed", function () {
		...
		deepEqual($pillbox.pillbox('items')[0], { name: 'Acai', value: 'acai' }, 'pillbox returns item added after user clicks suggestion');

		$input.val('');
		$input.trigger( $.Event( "keydown", { keyCode: 97 } ) ); // numpad 1
		$input.trigger( $.Event( "keydown", { keyCode: 40 } ) ); // down
		$input.trigger( $.Event( "keydown", { keyCode: 13 } ) ); // enter
		deepEqual($pillbox.pillbox('items')[1], { name: 'Acai', value: 'acai' }, 'pillbox returns item added after user keys down to suggestions');
		...
		deepEqual($pillbox.pillbox('items')[2], { name: 'Acai', value: 'acai' }, 'pillbox returns item added after user keys up to suggestion');

		$input.val('');
		$input.trigger( $.Event( "keydown", { keyCode: 97 } ) ); // numpad 1
		$input.trigger( $.Event( "keydown", { keyCode: 9 } ) ); // tab
		$input.trigger( $.Event( "keydown", { keyCode: 13 } ) ); // enter
		deepEqual($pillbox.pillbox('items')[3], { name: 'Acai', value: 'acai' }, 'pillbox returns item added after user tabs down to suggestion');
	...
	test("Edit functionality should behave as designed", function () {
		...
		$input.val('test edit');
		$input.trigger( $.Event( "keydown", { keyCode: 13 } ) );
		deepEqual($pillbox.pillbox('items')[0], {name: 'test edit', value: 'test edit'}, 'pillbox item was able to be edited');
	});
	...
	test("Triggers behave as designed", function () {
		...
		$pillbox.on('clicked.fu.pillbox', function( ev, item ){
			deepEqual(item, {name: 'Item 1', value: 'foo'}, 'clicked event is triggered');
		});
		$pillbox.find('> ul > li:first span:first').click();
		$pillbox.off('clicked.fu.pillbox');

		$pillbox.on('added.fu.pillbox', function( ev, item ){
			deepEqual(item, {name: 'added test', value: 'added test'}, 'added event is triggered');
		});
		...
		$pillbox.on('removed.fu.pillbox', function( ev, item ){
			deepEqual(item, {name: 'added test', value: 'added test'}, 'removed event is triggered');
		});
		...
		$pillbox.on('edited.fu.pillbox', function( ev, item ){
			deepEqual(item, {name: 'edit test', value: 'edit test'}, 'edit event is triggered');
		});
