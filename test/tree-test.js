...
define(function treeTest (require) {
	...
	var helperData = require('./data/treeData');

	QUnit.module('Fuel UX Tree', {
		beforeEach: function generateGUIDandDataSource () {
			var callLimit = 50;
			var callCount = 0;

			this.dataSource = function genDataSource (options, callback) {
				if (callCount >= callLimit) {
					callback(helperData.callCountData, 400);
					return;
				}

				callCount++;

				callback(helperData.treeData());
			};

			this.textDataSource = function textDataSource (options, callback) {
				callback(helperData.textData);
			};
		}
	});
